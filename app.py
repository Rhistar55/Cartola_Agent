...
with tab_manual:
    ...
    for pos_id, qtd in contagem.items():
        ...
        for i in range(qtd):
            with cols[i]:
                ja_escolhidos_ids = [e.atleta_id for e in escolhidos]
                ...
                if escolha_id is not None:
                    ...
                    st.caption(f"{adv_txt} · C$ {linha.preco_num:.2f} · previsão {linha.pred:.2f} pts")

    st.divider()
    total_slots = sum(contagem.values())
    if escolhidos:
        candidatos_cap = [e for e in escolhidos if e.posicao_id != 6]  # técnico não é capitão
        nome_cap = st.selectbox(
            "Capitão", ["— nenhum —"] + [f"{e.apelido} ({e.clube_abrev})" for e in candidatos_cap],
            key="capitao_manual",
        )
        custo = sum(e.preco_num for e in escolhidos)
        pontos = sum(e.pred for e in escolhidos)
        if nome_cap != "— nenhum —":
            cap = next(e for e in candidatos_cap if f"{e.apelido} ({e.clube_abrev})" == nome_cap)
            pontos += cap.pred * (mult_cap - 1)

        c1, c2, c3 = st.columns(3)
        c1.metric("Jogadores escalados", f"{len(escolhidos)}/{total_slots}")
        restante = cartoletas - custo
        c2.metric("Custo total", f"C$ {custo:.2f}",
                   delta=f"C$ {restante:.2f} livres" if restante >= 0 else f"estourou C$ {-restante:.2f}",
                   delta_color="normal" if restante >= 0 else "inverse")
        c3.metric("Pontuação prevista", f"{pontos:.2f} pts")

        if len(escolhidos) < total_slots:
            st.info(f"Faltam {total_slots - len(escolhidos)} jogador(es) para completar o time.")
        if custo > cartoletas:
            st.warning(f"Esse time estoura seu orçamento de C$ {cartoletas:.2f} em C$ {custo - cartoletas:.2f}.")
